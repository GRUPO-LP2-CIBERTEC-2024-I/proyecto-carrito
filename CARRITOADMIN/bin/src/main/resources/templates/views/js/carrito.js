...
function comprarProductos() {
    ...
    const precioTotal = document.querySelector('.price-total').textContent;

    // Crear un array de objetos solo con los datos necesarios para la compra
    const productosParaComprar = productosComprados.map(producto => {
        return {
            id: producto.id,
    ...
    const url = new URL('/front/comprar', window.location.origin);
    url.searchParams.append('total', precioTotal);
    url.searchParams.append('productos', JSON.stringify(productosParaComprar));

    // Redireccionar a la interfaz de compra con los datos
